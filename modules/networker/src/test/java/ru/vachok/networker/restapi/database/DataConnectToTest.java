...
public class DataConnectToTest {
    ...
    private static final TestConfigure TEST_CONFIGURE_THREADS_LOG_MAKER = new TestConfigureThreadsLogMaker(DataConnectToTest.class.getSimpleName(), System
        .nanoTime());

    private final DataConnectTo dataConnectTo = ru.vachok.networker.restapi.database.DataConnectTo.getInstance(ConstantsFor.DBBASENAME_U0466446_PROPERTIES);
    ...
    public void testGetDefaultConnection() {
        try (Connection connection = ru.vachok.networker.restapi.database.DataConnectTo.getDefaultI().getDataSource().getConnection()) {
            DatabaseMetaData connectionMetaData = connection.getMetaData();
            String metaDataURL = connectionMetaData.getURL();
            Assert.assertEquals(metaDataURL, "jdbc:mysql://srv-inetstat.eatmeat.ru:3306/velkom");
        }
