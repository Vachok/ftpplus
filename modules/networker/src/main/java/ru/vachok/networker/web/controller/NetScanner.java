...
//      model.addAttribute("pc", "<p>" + Arrays.toString(pcNames.toArray()).replaceAll(", ", "<br>") + "</p>");
    ...
    private String writeDB( Collection<String> pcNames ) {
        ...
        try (Connection c = dataConnectTo.getDefaultConnection("u0466446_velkom"); PreparedStatement p = c.prepareStatement("insert into  velkompc (NamePP, AddressPP, SegmentPP , OnlineNow) values (?,?,?,?)")) {
            pcNames.stream().sorted().forEach(x -> {
                String pcSerment = "Я не знаю...";
                String pcSerment1 = "Где-то в производстве";
                logger.info(x);
                ...
                if (x.contains("200.205")) pcSerment = pcSerment1;
                if (x.contains("200.206")) pcSerment = pcSerment1;
                if (x.contains("200.207")) pcSerment = "Сырокопоть";
                if (x.contains("200.208")) pcSerment = pcSerment1;
                if (x.contains("200.209")) pcSerment = pcSerment1;
