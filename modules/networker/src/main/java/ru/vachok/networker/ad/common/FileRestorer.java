...
public class FileRestorer extends SimpleFileVisitor<Path> implements Callable<List<?>> {
    ...
    @Override public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
        ...
        if (isFileTimeNewerThatPeriod & isFileNameContainsPattern) {
            ...
            try {
                isCopied = FileSystemWorker.copyOrDelFile(file.toFile(), pathToCopy, false);
            }
    ...
    private void checkFilesForRestore() throws InvokeIllegalException {
        for (Path archivedFile : archivedFiles) {
            if (archivedFile.getFileName().toString().toLowerCase().contains(restoreFilePattern.getFileName().toString())) {
                ...
                if (isTime) {
                    Path copyPath = Paths
                        .get(restoreFilePattern.toAbsolutePath().normalize().toString()
                            .replace(restoreFilePattern.getFileName().toString(), archivedFile.getFileName().toString()));
