...
public class DataSynchronizer extends SyncData {
    ...
    private final Queue<JsonObject> jsonObjects = new LinkedList<>();
    ...
    public String syncData() {
        int lastDBID = getLastLocalID(dbToSync);
        if (lastDBID == -666) {
            throw new IllegalStateException(getClass().getSimpleName());
        }
        final String sql = String.format("SELECT * FROM %s WHERE %s > %d", dbToSync, columnName, lastDBID);
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(sql).append("\n");
        int uploadedCount;

        if ((!dbToSync.contains(".inetstats") && !dbToSync.contains(ConstantsFor.DB_PCUSERAUTO_FULL))) {
            stringBuilder.append(connectToSQL(sql));
        }
    ...
    private String connectToSQL(final String sql) {
        String retStr = "null";
        try (Connection connection = DataConnectTo.getInstance(DataConnectTo.DEFAULT_I).getDefaultConnection(dbToSync)) {
            try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
                ...
                retStr = Arrays.toString(columns) + "\n";
    ...
    private List<String> getTblNames(String dbName) {
        ...
        try (Connection connection = dataConnectTo.getDefaultConnection(dbToSync)) {
            try (PreparedStatement preparedStatement = connection.prepareStatement(String.format("SHOW TABLE STATUS FROM `%s`", dbName))) {
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    preparedStatement.setQueryTimeout((int) TimeUnit.MINUTES.toSeconds(7));
                    while (resultSet.next()) {
